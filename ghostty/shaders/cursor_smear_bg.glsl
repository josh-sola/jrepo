...
// A trail painted in the background colour would be invisible, so keep only the
// hue and rebuild it bright enough to show against that background.
const float TRAIL_SATURATION = 0.85;
const float TRAIL_BRIGHTNESS = 0.95;

// Below this the background is near-grey and its hue is meaningless noise.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // This uniform tracks OSC 11, so the trail follows a background set at runtime.
    vec3 backgroundHsv = rgbToHsv(iBackgroundColor);
    vec4 tintedColor = vec4(
        hsvToRgb(vec3(backgroundHsv.x, TRAIL_SATURATION, TRAIL_BRIGHTNESS)),
        1.0
    );

    // An untinted background has no hue to borrow, so fall back to the cursor.
    float gray = dot(iCurrentCursorColor.rgb, vec3(0.299, 0.587, 0.114));
    const float saturationBoost = 1.8;
    vec4 cursorColor = clamp(
        mix(vec4(vec3(gray), iCurrentCursorColor.a), iCurrentCursorColor, saturationBoost),
        0.0, 1.0
    );

    vec4 enhancedColor = backgroundHsv.y < MIN_USABLE_SATURATION ? cursorColor : tintedColor;
