...
workspace opens with a first tab named `code`: an `emacsclient -t` frame with
the tree added as a treemacs project, and the agent gets a second, focused tab; an existing workspace just gets a new
