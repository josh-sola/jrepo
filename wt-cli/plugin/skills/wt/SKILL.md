...
`wt` manages adopted repositories and disposable Git worktrees. Use canonical
...
| `wt tree rm <TREE> [--force] [--delete-branch]` | Remove a tree after safety checks. |
...
| `wt upkeep doctor [--fix]` | Report registry drift and repair what `--fix` can. |
| `wt adopt-branch <BRANCH> [--repo <REPO>] [--name <SUMMARY>] [--profile <PROFILE,...>]` | Materialize a tree for an existing local branch. |
...
branch name. `<TREE_OR_REF>` also accepts a local branch or commit. Ambiguity
is an error; never guess.

## Workflow

```sh
wt tree new myrepo --name "feature base"
wt tree wait "feature base"
# edit and commit inside the tree
wt tree new myrepo --name "follow-up" --onto "feature base"
```

`wt tree new` can return before provisioning completes. Run `wt tree wait`
