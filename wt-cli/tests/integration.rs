...
#[test]
fn recursive_help_shows_the_plain_git_hierarchy() {
...
fn go_fails_before_any_hook_or_agent_when_planter_is_not_on_path() {
    ...
    assert_success(
        &run_wt(
            &root,
            &["tree", "new", "myrepo", "--name", "no planter target"],
        ),
        "new",
    );
    assert_success(
...
fn go_fails_when_planter_resolve_color_reports_an_error() {
    ...
    assert_success(
        &run_wt(
            &root,
            &["tree", "new", "myrepo", "--name", "resolver target"],
        ),
        "new",
    );
...
fn herdr_places_by_opening_a_worktree_when_no_workspace_is_open() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            format!("pane run w1:p1 'emacsclient' '-t' '{tree_path}'"),
